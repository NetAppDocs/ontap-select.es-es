...
Existen dos modelos de implementación de ONTAP Select HA: los clústeres multinodo (cuatro, seis, ocho, diez o doce nodos) y los clústeres de dos nodos. La característica principal de un clúster ONTAP Select de dos nodos es el uso de un servicio de mediación externo para resolver escenarios de cerebro dividido.  La máquina virtual ONTAP Deploy actúa como mediador predeterminado para todos los pares HA de dos nodos que configura.
