...
La NVRAM se ubica en su propio VMDK. Dividir la NVRAM en su propio VMDK permite que la VM ONTAP Select utilice el controlador vNVMe para comunicarse con su VMDK de NVRAM . También requiere que la máquina virtual ONTAP Select utilice la versión de hardware 13, que es compatible con ESX 8.0 y versiones posteriores.
