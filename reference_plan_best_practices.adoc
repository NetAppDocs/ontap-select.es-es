...
NetApp recomienda utilizar ESX 8.0 o posterior y un disco NVMe para el almacenamiento de datos que aloja los discos del sistema. Esta configuración proporciona el mejor rendimiento para la partición NVRAM .


NOTE: Al instalarse en ESX 8.0 o posterior, ONTAP Select utiliza el controlador vNVMEE independientemente de si el disco del sistema reside en una unidad SSD o en un disco NVMe. Esto establece el nivel de hardware de la máquina virtual en 13, compatible con ESX 8.0 y versiones posteriores.
