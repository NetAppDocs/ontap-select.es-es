...
---
= Protección de datos y eficiencia de ONTAP Select
:hardbreaks:
:allow-uri-read: 
:nofooter: 
:icons: font
...
[role="lead"]
ONTAP Select se basa en el software de almacenamiento ONTAP para ofrecer servicios de almacenamiento empresarial de forma eficiente mediante una arquitectura de escalado horizontal de alta disponibilidad y sin recursos compartidos. Puedes implementar una solución con uno, dos, cuatro, seis, ocho, diez o doce nodos, con hasta 400TB de capacidad bruta por nodo para almacenamiento conectado mediante NFS, SMB/CIFS e iSCSI. Puedes aprovechar la deduplicación y la compresión nativas para reducir los costes de almacenamiento al aumentar tu capacidad efectiva. La arquitectura de escalado horizontal permite alta disponibilidad y acceso a los datos sin interrupciones para el equilibrio de carga o para dar servicio al hardware.
...
Para más prácticas recomendadas y otros requisitos, consulta las secciones link:concept_ha_config.html#two-node-ha-versus-multi-node-ha["Alta disponibilidad de dos nodos frente a la alta disponibilidad de varios nodos"] y link:reference_plan_best_practices.html#two-node-stretched-ha-metrocluster-sds-best-practices["Prácticas recomendadas de alta disponibilidad (SDS de MetroCluster) extendidas de dos nodos"].
