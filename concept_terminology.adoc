...
Clúster de ONTAP Select:: Puede crear un clúster _ONTAP Select_ compuesto por uno, dos, cuatro, seis, ocho, diez o doce nodos. Los clústeres multinodo siempre contienen uno o más pares de alta disponibilidad (HA). Por ejemplo, un clúster de cuatro nodos consta de dos pares de HA. Un clúster de un solo nodo no ofrece capacidad de alta disponibilidad (HA).
...
Para más información, consulta las dos secciones link:concept_workflow_deploy.html["Planificación"] y link:concept_lic_evaluation.html["Licencia"].
...
Almacén de credenciales:: El almacén de credenciales de Deploy es una base de datos segura que contiene credenciales de cuentas. Se utiliza principalmente para registrar hosts de hipervisor como parte de la creación de un nuevo clúster. Consulta la sección link:concept_workflow_deploy.html["Planificación"] para más información.
...
+
--
Las configuraciones predeterminadas de vNAS habilitan una función de optimización de escritura conocida como registro de datos de instancia única (SIDL). Con ONTAP Select 9.6 y versiones posteriores, las funciones de eficiencia de almacenamiento en segundo plano de ONTAP están calificadas con SIDL habilitado. Consulta la sección link:concept_stor_concepts_chars.html["En profundidad"] para más información.
